...
async function updateStatusBarItem() {
    const config = vscode.workspace.getConfiguration('osMonitor');
    const showOS = config.get('showOS', true);
    const showCPU = config.get('showCPU', true);
    const showRAM = config.get('showRAM', true);
    const showDisk = config.get('showDisk', true);
    const showTraffic = config.get('showTraffic', true);
    const showBandwidth = config.get('showBandwidth', true);
    const iconOnly = config.get('iconOnly', false);
    ...
    const parts = [];
    if (showOS)
        parts.push(`$(desktop-download) ${osName}`);
    if (showCPU)
        parts.push(`$(pulse) ${iconOnly ? '' : 'CPU '}${resources.cpu}%`);
    if (showRAM)
        parts.push(`$(server) ${iconOnly ? '' : 'RAM '}${ramPercent}`);
    if (showDisk)
        parts.push(`$(database) ${iconOnly ? '' : 'DISK '}${diskPercent}`);
    if (showTraffic)
        parts.push(`$(arrow-swap) ${iconOnly ? '' : 'TRAF '}${resources.traf}`);
    if (showBandwidth)
        parts.push(`$(dashboard) ${iconOnly ? '' : 'BW '}${bwStr}`);
...
function calculateCpuUsage(currentCpus) {
    let idleDiff = 0;
    let totalDiff = 0;
    for (let i = 0; i < currentCpus.length; i++) {
        ...
        for (const type in current.times) {
            currentTotal += current.times[type];
        }
